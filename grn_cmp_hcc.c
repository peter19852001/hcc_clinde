 ...
 */
  int r=0, ifrom=0, ito=0, idelay=0, ieffect=0;
#define MAX_BUF 64
  ...
  return ifrom+ito+idelay+ieffect;
}
...
/* A simple wrapper */
edge* read_grn(char* name, int* out_n_edges, int* out_n_vertices, int* out_max_dely) {
  /* To read the edges and gives some useful information. The read
     edges should be freed after use. */
  int i=0, d=0;
  edge* r=NULL;
  clear_all_edges();
  read_edges_from_file(name);
...
void cmp_grn(char* pred_f, char* true_f, int n_non_hidden, int verbose) {
  ...
  */
  ...
  /* read predicted grn */
  pred_grn = read_grn(pred_f, &nL_pred, &nv_pred, &md_pred);
  if(verbose) {
  ...
  /* read true grn */
  true_grn = read_grn(true_f, &nL_true, &nv_true, &md_true);
  if(verbose) {
  ...
  */
  ...
  if(verbose) printf("==== Comparison\n==== link.r,link.p,link.f,delay.r,delay.p,delay.f,effect.r,effect.p,effect.f\n");
  printf("%f,%f,%f,%f,%f,%f,%f,%f,%f\n",
	 nL_r,nL_p,cal_fmeasure(nL_r,nL_p),
	 nD_r,nD_p,cal_fmeasure(nD_r,nD_p),
	 nE_r,nE_p,cal_fmeasure(nE_r,nE_p) );
...
int main(int argc, char *argv[])
{
  ...
  cmp_grn(pred_grn_file_name, true_grn_file_name, n_non_hidden, verbose);
